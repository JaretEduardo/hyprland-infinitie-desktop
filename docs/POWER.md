...
`config/hyprlock/hyprlock.conf` renders: the current wallpaper heavily blurred
(`$lock_image` — the still image, or a cached frame of the video wallpaper;
hyprlock never plays video), a time-of-day greeting (`hypr-greeting`), the
username large (`whoami` — never hardcoded), the weekday + date top-left, an
optional now-playing line (`playerctl`, hidden when nothing plays), and a clean
password field. Colours are the wallpaper-driven `$lock_*` vars
(`hyprlock-colors.local.conf`, regenerated by `hypr-wallpaper`). An optional
weather line appears only if you create `~/.config/hypr/weather.conf` with
`LOCATION="…"` — that enables one opt-in HTTPS request to `wttr.in` at lock
time (`scripts/desktop/hypr-weather`); with no file, no request and no line.
